...
import { CHAT_PAGE_TEXT, CHAT_QUICK_QUESTIONS } from '../locales/featureText';
...
const getStageDoneLabel = (step: ProgressStep, text: any): string => {
  const stage = step.stage || 'stage';
  if (step.message) return step.message;
  if (isStageDoneSuccessful(step.status)) return formatUiText(text.stageDone, { stage });
  return formatUiText(text.stageFinished, { stage, status: step.status || 'finished' });
};

const getPipelineBudgetSkippedLabel = (step: ProgressStep, text: any): string => {
  if (step.message) return step.message;
  return formatUiText(text.budgetSkipped, { stage: step.stage || 'pipeline' });
};
...
const ChatPage: React.FC = () => {
  ...
  const text = CHAT_PAGE_TEXT[language];
    ...
    async (stockCode: string) => {
      ...
      try {
        ...
        if (existingStockCode) {
          ...
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(formatUiText(text.watchlistRemoved, { code: stockCode }));
          }
        } else {
          const codes = await systemConfigApi.addToWatchlist(stockCode);
          if (isMountedRef.current) {
            setWatchlistCodes(codes);
            setWatchlistMessage(formatUiText(text.watchlistAdded, { code: stockCode }));
          }
        }
      } catch {
        if (isMountedRef.current) {
          setWatchlistMessage(text.watchlistFailed);
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || text.readCompressionFailed);
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || text.saveCompressionFailed);
      } finally {
    ...
    [
      ...
      text.saveCompressionFailed,
    ],
  );

  const availableSkillIds = new Set(skills.map((skill) => skill.id));
  const quickQuestions = CHAT_QUICK_QUESTIONS[language].filter((question) => availableSkillIds.size === 0 || availableSkillIds.has(question.skill));
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : [text.general];
    ...
    [activeStockContext, getSkillNames, input, loading, normalizeSelectedSkillIds, requestScrollToBottom, selectedSkillIds, sessionId, startStream, text.general],
  );
  ...
  const handleQuickQuestion = (q: { label: string; skill: string }) => {
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? text.userMessageHeader : `${text.aiReplyHeader}${skillLabel ? ` · ${skillLabel}` : ''}`;
  ...
  }, [text.aiReplyHeader, text.userMessageHeader]);

  const getCurrentStage = (steps: ProgressStep[]): string => {
    if (steps.length === 0) return text.connecting;
    const last = steps[steps.length - 1];
    if (last.type === 'thinking') return last.message || text.aiThinking;
    if (last.type === 'tool_start')
      return `${last.display_name || last.tool}...`;
    if (last.type === 'tool_done')
      return `${last.display_name || last.tool} ${text.completed}`;
    if (last.type === 'stage_start')
      return last.message || formatUiText(text.stageStart, { stage: last.stage || 'stage' });
    if (last.type === 'stage_done')
      return getStageDoneLabel(last, text);
    if (last.type === 'pipeline_timeout')
      return last.message || `${last.stage || 'pipeline'} timed out`;
    if (last.type === 'pipeline_budget_skipped')
      return getPipelineBudgetSkippedLabel(last, text);
    if (last.type === 'generating')
      return last.message || text.generatingFinal;
  ...
  const renderThinkingBlock = (msg: Message) => {
    ...
    const summary = formatUiText(text.toolCallsCount, {
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">{text.thinkingProcess}</span>
    ...
    <div className="mb-3 pl-5 border-l border-border/40 space-y-1.5 animate-fade-in">
      {steps.map((step, idx) => {
        ...
        if (step.type === 'thinking') {
          progressText = step.message || formatUiText(text.stepThinking, { step: step.step ?? '' });
          statusClass = 'chat-progress-item-thinking';
          iconClass = 'chat-progress-dot-thinking';
        } else if (step.type === 'tool_start') {
          progressText = `${step.display_name || step.tool}...`;
          statusClass = 'chat-progress-item-tool';
          iconClass = 'chat-progress-dot-tool';
        } else if (step.type === 'tool_done') {
          progressText = `${step.display_name || step.tool} (${step.duration}s)`;
          ...
          progressText = step.message || `Starting ${step.stage || 'stage'}...`;
          statusClass = 'chat-progress-item-thinking';
          iconClass = 'chat-progress-dot-thinking';
        } else if (step.type === 'stage_done') {
          const isSuccess = isStageDoneSuccessful(step.status);
          progressText = getStageDoneLabel(step, text);
        ...
        return (
          ...
          >
            ...
            <span className="leading-relaxed">{progressText}</span>
          </div>
        );
      })}
    </div>
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={formatUiText(text.switchChat, { title: s.title })}
                ...
                >
                  ...
                  <div className="content">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      <span className="meta">
                        {formatUiText(text.messageCount, { count: s.message_count })}
                      </span>
                      {s.last_active && (
                        <>
                ...
                <button
                  ...
                  aria-label={formatUiText(text.deleteChatAria, { title: s.title })}
                >
                  <svg
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        message={text.deleteChatMessage}
        confirmText={text.deleteChatConfirm}
        cancelText={text.deleteChatCancel}
        isDanger
        ...
        <header className="mb-4 flex-shrink-0 space-y-3">
          <div className="flex items-start justify-between gap-4">
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              ...
              {text.documentTitle.replace(' - DSA', '')}
            </h1>
            {messages.length > 0 && (
              <div className="flex flex-shrink-0 flex-wrap items-center justify-end gap-2">
                <Tooltip content={text.exportChatMarkdown}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      onClick={() => downloadSession(messages)}
                      aria-label={text.exportChatMarkdown}
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <Tooltip content={text.sendTooltip}>
                  <span className="inline-flex">
                    <Button
                      variant="action-primary"
                      size="sm"
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          showSendFeedback({ type: 'success', message: text.sendSuccess }, 3000);
                        } catch (err) {
                          const parsed = getParsedApiError(err);
                          showSendFeedback({
                            type: 'error',
                            message: parsed.message || text.sendFailed,
                          }, 5000);
                        } finally {
                          setSending(false);
                        }
                      }}
                      aria-label={text.sendTooltip}
                    >
                      {sending ? (
                        <svg
          ...
          {sendToast ? (
            <InlineAlert
              ...
              title={sendToast.type === 'success' ? text.sendSuccessTitle : text.sendFailed}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title={text.startChatTitle}
                  description={text.startChatDescription}
                      ...
                      <div className="relative">
                        <div className="chat-message-actions">
                          ...
                          <button
                            ...
                            className="chat-copy-btn"
                            aria-label={text.exportMessageMarkdown}
                          >
                            {text.exportMessage}
                          </button>
                        </div>
          ...
          {showJumpToBottom && (
            <div className="pointer-events-none absolute bottom-[5.75rem] right-4 z-20 md:bottom-24 md:right-6">
              <button
                ...
                aria-label={text.jumpToBottomAria}
              >
                <svg
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  variant="info"
                  title={text.followUpLoadingTitle}
                  message={text.followUpLoadingMessage}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  />
                  <span className="font-medium">{text.contextCompression}</span>
                  <span className="text-xs text-muted-text">{text.contextCompressionDesc}</span>
                </label>
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title={text.compressionNotSavedTitle}
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  <button
                    ...
                    aria-label={mobileSkillPickerOpen ? text.collapseSkills : text.expandSkills}
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="flex-shrink-0 font-medium">{text.skills}</span>
            ...
            {activeStockCode && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-[11px]"
                >
                  {stockInWatchlist(activeStockCode) ? text.removeFromWatchlist : text.addToWatchlist}
                </Button>
                {watchlistMessage && (
